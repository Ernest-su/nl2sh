...
颜色原则上只表达语义，不用于装饰整段内容。明确的例外是欢迎页与帮助页中的佛祖字符画，以及欢迎页启动小火车；它们可组合主题 palette 表现插图层次，颜色不携带任何状态或风险含义。普通正文保持灰白；高饱和度颜色只标记标题、焦点、状态、风险及这些明确限定的插图。任何组件不得自行定义与本文冲突的 RGB 或 ANSI 色值。
...
- 首次启动欢迎页可在佛祖字符画下方播放一次纯 ASCII 小火车动画：车身必须包含金色加粗的 `NL2SH`，车顶、车身、车厢、轮组和烟雾可组合 `decorative_gold`、`accent`、`cyan`、`special` 与中性文字色形成彩色层次，但不得借用 `warning`/`error` 暗示状态。烟雾循环变化，列车从内容区左侧完整驶入并从右侧完整消失。动画不得写入会话或模型历史，不在 `/help` 中重新触发，并须保持输入和退出操作实时可用；窄屏按内容区宽度裁剪，不得产生横向换行或破坏边框。
