...
pub(crate) fn startup_history(language: UiLanguage, ascii: bool) -> Vec<String> {
    ...
    let mut history = match language {
        UiLanguage::ZhCn => vec![
            ...
            format!("{hint} 配置命令：/provider 配置 API；/model 手工配置模型；/models 在线选择模型；/proxy 配置代理"),
        ...
        UiLanguage::En => vec![
            ...
            format!("{hint} Setup: /provider configures the API; /model enters a model manually; /models fetches models; /proxy configures a proxy"),
...
pub(crate) fn help_history(language: UiLanguage, ascii: bool) -> Vec<String> {
    ...
    let mut history = match language {
        UiLanguage::ZhCn => vec![
            ...
            format!("{hint} /models 从 Provider 拉取并选择模型，失败时可手工输入"),
            format!("{hint} /balance 查询支持的 Provider 余额，结果不写入日志"),
            format!("{hint} /proxy 在 TUI 弹窗中配置 HTTP/SOCKS 代理"),
        ...
        UiLanguage::En => vec![
            ...
            format!("{hint} /models fetch and select provider models, with manual fallback"),
            format!("{hint} /balance query supported provider balances without logging them"),
            format!("{hint} /proxy configure an HTTP/SOCKS proxy in a TUI popup"),
