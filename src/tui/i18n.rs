...
pub(crate) fn startup_history(language: UiLanguage, ascii: bool) -> Vec<String> {
    ...
    let mut history = match language {
        UiLanguage::ZhCn => vec![
            ...
            format!("{hint} 配置命令：/provider 配置 API；/model 手工配置模型；/models 在线选择模型"),
        ...
        UiLanguage::En => vec![
            ...
            format!("{hint} Setup: /provider configures the API; /model enters a model manually; /models fetches models"),
